"""Module 02 solution: from zero-code gateway traces to a trace that reads agent -> tool -> llm.
...
def step_1_zero_code() -> None:
    ...
    print(f"    same thing from the CLI: orq traces search --from now-10m --to now --json | jq '.data[] | {{trace_id, name}}'")
...
def step_4_annotation(trace_id: str, span_id: str) -> None:
    """Annotations need a definition in the workspace. `rating` exists here; a missing key is a 404."""
    try:
