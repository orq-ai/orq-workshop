...
Fill the TODOs; the solution is in solution/run.py.
"""
...
def step_2_launch_dry_run() -> None:
    for agent in ["claude"]:  # TODO: add opencode; note the env vars each agent gets
...
def step_4_sessions_as_traces() -> None:
    ...
    for t in data:
        if t["name"].startswith("messages.anthropic"):  # TODO: also match attributes.orq.coding_assistant (Codex, others)
    ...
    for sid, calls in sessions.items():
        ...
        print(f"    session {sid}  calls={len(calls)} cost=${cost:.4f}")  # TODO: add tokens and cached tokens
