...
|---|---|
| **Time** | 15 min |
| **Prerequisites** | module 00; the [orqi harness](https://orq-ai.github.io/orq-workshop/setup/orqi/) page (orqi installed and signed in); traffic from modules 01 to 04 in the workspace (or `make traffic`) |
| **You will have** | four real answers from orqi about your own workspace, a diagnosis of a seeded client-side failure, and a clear rule for when to reach for orqi versus `orq launch`. |
...
orqi has the same 43 `orq_` tools and the same skills your coding agent gets in module 13, plus seven of its own (`workspace-health-check`, `investigate-root-cause`, `debug-conversation`, `optimize-cost`, `setup-guardrails`, `manage-knowledge-base`, `platform-guide`) and three subagents (`investigator`, `analyst`, `docs`). What it lacks is a repo to build: it reads and runs shell commands in your current directory, but its job is the platform. Install and sign-in are on the [orqi harness](https://orq-ai.github.io/orq-workshop/setup/orqi/) page; this module assumes the header line already shows `43 tools` and `ORQ_API_KEY`.
...
- Errors are mostly **HTTP 408 timeouts** on `openai/gpt-5.6-sol`.
...
## Recommended Actions
1. **Investigate the `gpt-5.6-sol` timeout cluster first.** Use `investigate-root-cause` on trace `1ef2bd90187a45fe20d13c52c15475d3`.
```
...
| Invalid/unavailable model ID | 1 | HTTP 404; `openai/gpt-5.6-luna-does-not-exist` |
...
| Step | Result |
| Initial model: `openai/gpt-5.6-sol` | Timed out after ~1502 ms |
| Retry: `gpt-5.6-sol` | Timed out after ~1502 ms |
| Fallback: `openai/gpt-5.4-nano` | Timed out twice, each ~1502 ms |
| Fallback: `openai/gpt-5.6-luna` | Timed out twice, each ~1502 ms |
...
### Step 2 · A failure orqi cannot see in Traces
...
```bash
$ orqi "make smoke in my repo fails with this error, diagnose it and tell me what to fix: openai.NotFoundError: Error code: 404 on POST https://my.orq.ai/broken/responses, empty body, no x-orq-trace-id header. The app uses the OpenAI SDK with base_url built from ORQ_BASE_URL in .env."
```
...
So the expected final endpoint is: https://my.orq.ai/v3/router/responses
Your failing endpoint was:         https://my.orq.ai/broken/responses
...
### Step 3 · Platform questions go to the docs
...
- orqi runs shell commands in your current directory. It ran `make smoke` on its own in Step 2.
...
- `.env` overrides the shell by design, which is why Step 2 breaks the client in code rather than through a temp env file.
